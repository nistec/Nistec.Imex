...
namespace Nistec.Imex.MSExcel.Bin2003
{
    ...
    internal class ExcelHeader
    {
        ...
        private ExcelHeader(Stream file)
        {
        ...
        /// <returns>ExcelHeader representing specified file</returns>
        public static ExcelHeader ReadHeader(Stream file)
        {
            ExcelHeader hdr = new ExcelHeader(file);
            lock (file)
            {
    ...
    internal class XlsFat
    {
        ...
        private ExcelHeader m_hdr;

        /// <summary>
        /// Constructs FAT table from list of sectors
        /// </summary>
        /// <param name="hdr">ExcelHeader</param>
        /// <param name="sectors">Sectors list</param>
        public XlsFat(ExcelHeader hdr, List<uint> sectors)
        {
            m_hdr = hdr;
        ...
        public uint GetNextSector(uint sector)
        {
            if (m_fat.Count <= sector)
                throw new ArgumentOutOfRangeException(" There's no such sector in FAT.");
        ...
        /// <summary>
        /// Returns underlying ExcelHeader object
        /// </summary>
    ...
    internal class XlsStream
    {
        ...
        protected ExcelHeader m_hdr;
        protected uint m_startSector;

        public XlsStream(ExcelHeader hdr, uint startSector)
        {
        ...
        //write
        //public void WriteStream(byte[] buff)
        //{
        //    uint sector = m_startSector, prevSector = 0;
        //    int sectorSize = m_hdr.SectorSize;
        //    //byte[] buff = new byte[sectorSize];
        ...
        //                m_file.Seek((sector + 1) * sectorSize, SeekOrigin.Begin);
        //            prevSector = sector;
        //            m_file.Write(buff, 0, sectorSize);
        //        }
        //        while ((sector = m_fat.GetNextSector(sector)) != (uint)FATMARKERS.FAT_EndOfChain);
        //    }
        //}
    ...
    internal class XlsRootDirectory
    {
        ...
        /// <summary>
        /// Creates Root Directory catalog from ExcelHeader
        /// </summary>
        /// <param name="hdr">ExcelHeader object</param>
        public XlsRootDirectory(ExcelHeader hdr)
        {
            m_hdr = hdr;
    ...
    /// </summary>
    internal class ExcelStream : XlsStream
    {
        private byte[] bytes;
        private int m_offset;
        private int m_size;

        public ExcelStream(ExcelHeader hdr, uint streamStart)
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public ExcelRecord Read()
        {
            ExcelRecord rec = ExcelRecord.GetRecord(bytes, (uint)m_offset);
            m_offset += rec.Size;
            if (m_offset > m_size)
                return null;
            return rec;
        ...
        /// <returns></returns>
        public ExcelRecord ReadAt(int offset)
        {
            ExcelRecord rec = ExcelRecord.GetRecord(bytes, (uint)offset);
    ...
    internal class ExcelRecord
    {
        ...
        protected ExcelRecord(byte[] bytes) : this(bytes, 0) { }

        protected ExcelRecord(byte[] bytes, uint offset)
        {
        ...
        /// <returns></returns>
        public static ExcelRecord GetRecord(byte[] bytes, uint offset)
        {
            ...
            switch ((BIFFRECORDTYPE)ID)
            {
                ...
                case BIFFRECORDTYPE.BOF:
                    return new ExcelBOF(bytes, offset);
                case BIFFRECORDTYPE.EOF:
                    return new ExcelEOF(bytes, offset);
                case BIFFRECORDTYPE.INTERFACEHDR:
                    return new ExcelInterfaceHdr(bytes, offset);

                case BIFFRECORDTYPE.SST:
                    return new ExcelSST(bytes, offset);

                case BIFFRECORDTYPE.INDEX:
                    return new ExcelIndex(bytes, offset);
                case BIFFRECORDTYPE.ROW:
                    return new ExcelRow(bytes, offset);
                case BIFFRECORDTYPE.DBCELL:
                    return new ExcelDbCell(bytes, offset);

                case BIFFRECORDTYPE.BLANK:
                case BIFFRECORDTYPE.BLANK_OLD:
                    return new ExcelBlankCell(bytes, offset);
                case BIFFRECORDTYPE.MULBLANK:
                    return new ExcelMulBlankCell(bytes, offset);
                case BIFFRECORDTYPE.LABEL:
                case BIFFRECORDTYPE.LABEL_OLD:
                case BIFFRECORDTYPE.RSTRING:
                    return new ExcelLabelCell(bytes, offset);
                case BIFFRECORDTYPE.LABELSST:
                    return new ExcelLabelSSTCell(bytes, offset);
                case BIFFRECORDTYPE.INTEGER:
                case BIFFRECORDTYPE.INTEGER_OLD:
                    return new ExcelIntegerCell(bytes, offset);
                case BIFFRECORDTYPE.NUMBER:
                case BIFFRECORDTYPE.NUMBER_OLD:
                    return new ExcelNumberCell(bytes, offset);
                case BIFFRECORDTYPE.RK:
                    return new ExcelRKCell(bytes, offset);
                case BIFFRECORDTYPE.MULRK:
                    return new ExcelMulRKCell(bytes, offset);
                case BIFFRECORDTYPE.FORMULA:
                case BIFFRECORDTYPE.FORMULA_OLD:
                    return new ExcelFormulaCell(bytes, offset);
                case BIFFRECORDTYPE.STRING:
                    return new ExcelFormulaString(bytes, offset);
                case BIFFRECORDTYPE.CONTINUE:
                    return new ExcelContinue(bytes, offset);
                case BIFFRECORDTYPE.DIMENSIONS:
                    return new ExcelDimensions(bytes, offset);
                case BIFFRECORDTYPE.BOUNDSHEET:
                    return new ExcelBoundSheet(bytes, offset);
                case BIFFRECORDTYPE.WINDOW1:
                    return new ExcelWindow1(bytes, offset);
                case BIFFRECORDTYPE.CODEPAGE:
                    return new ExcelSimpleValueRecord(bytes, offset);
                ...
                default:
                    return new ExcelRecord(bytes, offset);
            }
    ...
    /// </summary>
    internal class ExcelBOF : ExcelRecord
    {

        internal ExcelBOF(byte[] bytes) : this(bytes, 0) { }
        internal ExcelBOF(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelEOF : ExcelRecord
    {
        internal ExcelEOF(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelEOF(byte[] bytes) : this(bytes, 0) { }
    }

    /// <summary>
    /// Represents record with the only two-bytes value
    /// </summary>
    internal class ExcelSimpleValueRecord : ExcelRecord
    {
        internal ExcelSimpleValueRecord(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelSimpleValueRecord(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelInterfaceHdr : ExcelRecord
    {
        internal ExcelInterfaceHdr(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelInterfaceHdr(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelDimensions : ExcelRecord
    {
        internal ExcelDimensions(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelDimensions(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelIndex : ExcelRecord
    {
        internal ExcelIndex(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelIndex(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelSST : ExcelRecord
    {
        private List<string> m_strings;
        private uint m_size = 0;
        private List<uint> continues = new List<uint>();

        internal ExcelSST(byte[] bytes, uint offset)
            : base(bytes, offset)
        {
            m_size = RecordSize;
            m_strings = new List<string>((int)Count);
        }
        internal ExcelSST(byte[] bytes) : this(bytes, 0) { }
        ...
        //write
        //public void SetString(uint SSTIndex,string value)
        //{
        //    if (SSTIndex < m_strings.Count)
        //        m_strings[(int)SSTIndex]=value;
        //    else
        //        throw new ArgumentOutOfRangeException(SSTIndex.ToString());// string.Empty;
        //}
        ...
        public void Append(ExcelContinue fragment)
        {
    ...
    /// </summary>
    internal class ExcelContinue : ExcelRecord
    {
        internal ExcelContinue(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelContinue(byte[] bytes) : this(bytes, 0) { }
    }

    /// <summary>
    /// Represents row record in table
    /// </summary>
    internal class ExcelRow : ExcelRecord
    {
        internal ExcelRow(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelRow(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelDbCell : ExcelRecord
    {
        internal ExcelDbCell(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelDbCell(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelBlankCell : ExcelRecord
    {
        internal ExcelBlankCell(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelBlankCell(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelIntegerCell : ExcelBlankCell
    {
        internal ExcelIntegerCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelIntegerCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelMulBlankCell : ExcelBlankCell
    {
        internal ExcelMulBlankCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelMulBlankCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelNumberCell : ExcelBlankCell
    {
        internal ExcelNumberCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelNumberCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelLabelCell : ExcelBlankCell
    {
        internal ExcelLabelCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelLabelCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelRKCell : ExcelBlankCell
    {
        internal ExcelRKCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelRKCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelMulRKCell : ExcelBlankCell
    {
        internal ExcelMulRKCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelMulRKCell(byte[] bytes, uint offset) : base(bytes, offset) { }
        ...
        public double GetValue(ushort ColumnIdx)
        {
            ...
            if (ofs > RecordSize)
                return 0;
            return ExcelRKCell.NumFromRK(base.ReadUInt32(ofs));
        }
    ...
    /// </summary>
    internal class ExcelLabelSSTCell : ExcelBlankCell
    {
        internal ExcelLabelSSTCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelLabelSSTCell(byte[] bytes, uint offset) : base(bytes, offset) { }
        ...
        /// <returns></returns>
        public string Text(ExcelSST sst)
        {
    ...
    /// </summary>
    internal class ExcelBoolErrCell : ExcelBlankCell
    {
        internal ExcelBoolErrCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelBoolErrCell(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelFormulaString : ExcelRecord
    {
        internal ExcelFormulaString(byte[] bytes) : this(bytes, 0) { }
        internal ExcelFormulaString(byte[] bytes, uint offset) : base(bytes, offset) { }
    ...
    /// </summary>
    internal class ExcelFormulaCell : ExcelNumberCell
    {
        internal ExcelFormulaCell(byte[] bytes) : this(bytes, 0) { }
        internal ExcelFormulaCell(byte[] bytes, uint offset) : base(bytes, offset) { }
        ...
        public new object Value
        {
            get
            {
                ...
                if (((ulong)val & 0xFFFF000000000000) == 0xFFFF000000000000)
                {
                    ...
                    {
                        case 0:     // String
                            ExcelFormulaString str = ExcelRecord.GetRecord(m_bytes, (uint)(Offset + Size)) as ExcelFormulaString;
                            if (str == null)
                                return string.Empty;
                            else
                            {
    ...
    /// </summary>
    internal class ExcelWindow1 : ExcelRecord
    {
        [Flags()]
        ...
        internal ExcelWindow1(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelWindow1(byte[] bytes) : this(bytes, 0) { }
    ...
    /// </summary>
    internal class ExcelBoundSheet : ExcelRecord
    {
        ...
        internal ExcelBoundSheet(byte[] bytes, uint offset) : base(bytes, offset) { }
        internal ExcelBoundSheet(byte[] bytes) : this(bytes, 0) { }
    ...
    internal class ExcelWorkbook
    {
        ...
        private ExcelInterfaceHdr m_InterfaceHdr = null;

        public ExcelInterfaceHdr InterfaceHdr
        {
            get { return m_InterfaceHdr; }
            set { m_InterfaceHdr = value; }
        }

        private ExcelRecord m_MMS = null;

        public ExcelRecord MMS
        {
            get { return m_MMS; }
            set { m_MMS = value; }
        }

        private ExcelRecord m_WriteAccess = null;

        public ExcelRecord WriteAccess
        {
            get { return m_WriteAccess; }
            set { m_WriteAccess = value; }
        }

        private ExcelSimpleValueRecord m_CodePage = null;

        public ExcelSimpleValueRecord CodePage
        {
            get { return m_CodePage; }
            set { m_CodePage = value; }
        }

        private ExcelRecord m_DSF = null;

        public ExcelRecord DSF
        {
            get { return m_DSF; }
            set { m_DSF = value; }
        }

        private ExcelRecord m_Country = null;

        public ExcelRecord Country
        {
            get { return m_Country; }
            set { m_Country = value; }
        }

        private ExcelSimpleValueRecord m_Backup = null;

        public ExcelSimpleValueRecord Backup
        {
            get { return m_Backup; }
            set { m_Backup = value; }
        }

        private List<ExcelRecord> m_Fonts = new List<ExcelRecord>();

        public List<ExcelRecord> Fonts
        {
            get { return m_Fonts; }
        }

        private List<ExcelRecord> m_Formats = new List<ExcelRecord>();

        public List<ExcelRecord> Formats
        {
            get { return m_Formats; }
        }

        private List<ExcelRecord> m_ExtendedFormats = new List<ExcelRecord>();

        public List<ExcelRecord> ExtendedFormats
        {
            get { return m_ExtendedFormats; }
        }

        private List<ExcelRecord> m_Styles = new List<ExcelRecord>();

        public List<ExcelRecord> Styles
        {
            get { return m_Styles; }
        }

        private List<ExcelBoundSheet> m_Sheets = new List<ExcelBoundSheet>();

        public List<ExcelBoundSheet> Sheets
        {
            get { return m_Sheets; }
        }

        private ExcelSST m_SST = null;
        ...
        private ExcelRecord m_ExtSST = null;

        public ExcelRecord ExtSST
        {
            get { return m_ExtSST; }
            set { m_ExtSST = value; }
        }
    ...
    internal class ExcelWorksheet
    {
        ...
        public ExcelWorksheet(int index, ExcelBoundSheet refSheet)
        {
            m_Index = index;
        ...
        private ExcelSimpleValueRecord m_CalcMode = null;

        public ExcelSimpleValueRecord CalcMode
        {
            get { return m_CalcMode; }
            set { m_CalcMode = value; }
        }

        private ExcelSimpleValueRecord m_CalcCount = null;

        public ExcelSimpleValueRecord CalcCount
        {
            get { return m_CalcCount; }
            set { m_CalcCount = value; }
        }

        private ExcelSimpleValueRecord m_RefMode = null;

        public ExcelSimpleValueRecord RefMode
        {
            get { return m_RefMode; }
            set { m_RefMode = value; }
        }

        private ExcelSimpleValueRecord m_Iteration = null;

        public ExcelSimpleValueRecord Iteration
        {
            get { return m_Iteration; }
            set { m_Iteration = value; }
        }

        private ExcelRecord m_Delta = null;

        public ExcelRecord Delta
        {
            get { return m_Delta; }
            set { m_Delta = value; }
        }

        private ExcelDimensions m_Dimensions = null;

        /// <summary>
        /// Dimensions of worksheet
        /// </summary>
        public ExcelDimensions Dimensions
        {
            get { return m_Dimensions; }
            set { m_Dimensions = value; }
        }

        private ExcelRecord m_Window2 = null;

        public ExcelRecord Window2
        {
