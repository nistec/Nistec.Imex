namespace Nistec.Imex.ExcelXml
{
    ...
    public class Workbook
    {
        ...
        public static bool Export(DataTable table, string fileName, bool firstRowHeader, Nistec.Imex.Data.AdoField[] fields)
        {
            Workbook workbook = new Workbook();
        ...
        private static void Export(Workbook workbook, DataTable table, bool firstRowHeader, Nistec.Imex.Data.AdoField[] fields)
        {
            ...
            {
                fields = Nistec.Imex.Data.AdoField.CreateFields(table);
            }
            ...
            for (int i = 0; i < iWorkSheet; i++)
            {
                ...
                foreach (Nistec.Imex.Data.AdoField c in fields)
                {
                    if (writeHeader)
                    {
                        //Headers
        ...
        internal static void ReadNamedRanges(XmlReader reader, Workbook book, Worksheet ws)
        {
            if (!reader.IsEmptyElement)
            {
                while (reader.Read() && (!(reader.Name == "Names") || (reader.NodeType != XmlNodeType.EndElement)))
                {
                    ...
                    {
                        Nistec.Imex.ExcelXml.Range range = null;
                        string name = "";
                        ...
                        while (reader.MoveToNextAttribute()) //foreach (XmlReaderAttributeItem item in reader.GetAttributes())
                        {
                            ...
                            {
                                range = new Nistec.Imex.ExcelXml.Range(reader.Value);
                            }
                        }
        ...
        public void AddNamedRange(Nistec.Imex.ExcelXml.Range range, string name)
        {
        ...
        internal void AddNamedRange(Nistec.Imex.ExcelXml.Range range, string name, Worksheet ws)
        {
