        ...
        public AdoReader(ReaderType type, AdoProperties properties)
        {
            switch (type)
            {
                case ReaderType.Excel2003:
                    _reader = new Nistec.Imex.MSExcel.Bin2003.ExcelReader(properties as MSExcel.ExcelReadProperties);
                    break;
                case ReaderType.Excel2007:
                    throw new ArgumentException("Not implemented");
                case ReaderType.ExcelOleDb:
                    _reader = new Nistec.Imex.MSExcel.OleDb.ExcelReader(properties as MSExcel.ExcelReadProperties);
                    break;
                case ReaderType.ExcelXml:
                    _reader = new Nistec.Imex.MSExcel.Xml.ExcelReader(properties as MSExcel.ExcelReadProperties);
                    break;
                case ReaderType.Csv:
                    _reader = new Nistec.Imex.Csv.CsvReader(properties as Csv.CsvReadProperties);
                    break;
                case ReaderType.Xml:
                    _reader = new Nistec.Imex.Xml.XmlReader(properties as Xml.XmlReadProperties);
                    break;
                case ReaderType.Clipboard:
                    _reader = new Nistec.Imex.Clipboard.ClipboardReader(properties as Clipboard.ClipboardReadProperties);
                    break;
                default:
